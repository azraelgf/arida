(() => {
    ...
    class ScrollWatcher {
        ...
        scrollWatcherConstructor(items) {
            ...
            } else this.scrollWatcherLogging("Сплю, нет объектов для наблюдения. ZzzZZzz");
        }
        getScrollWatcherConfig(paramsWatch) {
            ...
            if (paramsWatch.margin.indexOf("px") < 0 && paramsWatch.margin.indexOf("%") < 0) {
                this.scrollWatcherLogging(`ой, параметр data-watch-margin нужно задавать в PX или %`);
                return;
            }
        ...
        scrollWatcherOff(targetElement, observer) {
            ...
            this.scrollWatcherLogging(`Перестал наблюдать за ${targetElement.classList}`);
        }
    ...
    modules_flsModules.popups = new Popups;
    class Modals {
        modals={
            "call-back": "call-back",
            photos: "photos",
        ...
        findUniversalModals() {
            let prefixList = [ "dostavka-kurerom-", "samovyvoz-", "payment-" ];
            document.querySelectorAll("[data-modal-name]").forEach(m => {
                if (!!m.dataset.modalName) {
                    ...
                    if (!!modalFinded && !this.modals[modalFinded]) this.modals[modalFinded] = modalFinded;
                }
            });
        }
        getButtons() {
            this.btn = this.btn ?? {};
            for (let m in this.modals) {
                let b1 = document.querySelectorAll('[href="' + this.modals[m] + '"]');
                let b2 = document.querySelectorAll('[data-modal-name="' + this.modals[m] + '"]');
                this.btn[m] = [ ...b1, ...b2 ];
            ...
            let btnOpener = e => {
                e.preventDefault();
                let bt, mKey;
                bt = e.target.closest(".modal-btn");
                mKey = bt.getAttribute("href");
                if (!mKey) mKey = bt.dataset.modalName;
                if (!this.mod[mKey]) return;
                console.log("Открываем модалку", mKey, this.mod);
                this.mod[mKey].o(this, bt);
            };
            for (let b in this.btn) this.btn[b].forEach(btn => {
                if (typeof btn.modalStatusActive != "undefined") return;
                btn.modalStatusActive = false;
        ...
        getCloseBtns() {
            this.cls = this.cls ?? {};
            for (let m in this.mod) this.cls[m] = document.getElementById(m).querySelector(".modal__close");
            let btnCloser = e => {
                e.preventDefault();
                let cb = e.target.closest(".modal__close");
                let mKey = cb.parentElement.closest(".modal").getAttribute("id");
                this.mod[mKey].c();
                this.mod[mKey].removeAfterClosing === true && setTimeout(() => {
                    this.mod[mKey].parentNode.removeChild(this.mod[mKey]);
                }, 350);
            };
        ...
        handleInner() {
            const innerCloser = e => {
                ...
                const mKey = modalEl.id;
                const clickedInsideBody = e.target.closest(`#${mKey} .modal__body`);
                if (!clickedInsideBody) {
                    this.mod[mKey].c(this);
                    if (this.mod[mKey].removeAfterClosing === true) setTimeout(() => {
                        this.mod[mKey].parentNode.removeChild(this.mod[mKey]);
                    }, 350);
                }
            };
            for (let key in this.mod) {
                const modal = this.mod[key];
                modal.removeEventListener("click", innerCloser);
                modal.addEventListener("click", innerCloser);
                modal.removeEventListener("touchstart", innerCloser);
                modal.addEventListener("touchstart", innerCloser, {
                    passive: true
                });
            }
        }
        o(mKey) {
            let eventOnBeforeOpenResult = this.events.onBeforeOpen.exec(mKey);
            if (!eventOnBeforeOpenResult) return;
        ...
        c(mKey) {
            if (this.mod[mKey].classList.contains("modal--active")) {
                let eventOnBeforeCloseResult = this.events.onBeforeClose.exec(mKey);
                if (!eventOnBeforeCloseResult) return;
                this.mod[mKey].classList.remove("modal--active");
                document.body.classList.remove("modalOpen");
                this.mod[mKey].isOpened = false;
                const anyOpen = Object.keys(this.mod).some(key => this.mod[key]?.isOpened);
                if (!anyOpen) document.documentElement.classList.remove("lock");
                this.events.onClose.exec(mKey);
            }
        }
        handleOpen(b) {
            b = this.mod[b];
            if (b.classList.contains("modal--active")) b.classList.remove("modal--active"); else {
                this.handleInner();
    ...
    modules_flsModules.modals = new Modals;
    document.addEventListener("click", e => {
    ...
    (function connectVideoToModals() {
        ...
        modules_flsModules.modals.events.onOpen.add(MODAL_ID, {
            fn: () => {
        ...
        modules_flsModules.modals.events.onClose.add(MODAL_ID, {
            fn: () => {
                const modal = document.getElementById(MODAL_ID);
                if (!modal) return true;
                const slot = modal.querySelector("[data-video-slot]");

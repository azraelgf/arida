(() => {
    ...
    const unicodeLength = str => Array.from(str).length;
    ...
    const asciifold = str => Array.from(str).reduce((result, char) => result + _asciifold(char), "");
    ...
    class Sifter {
        ...
        _getScoreFunction(search) {
            ...
            const scoreObject = function() {
                ...
                return function(token, data) {
                    ...
                    } else iterate(weights, (weight, field) => {
        ...
        search(query, options) {
            ...
            if (query.length) iterate(self.items, (item, id) => {
                score = fn_score(item);
                if (options.filter === false || score > 0) search.items.push({
                    score,
                    id
                });
            }); else iterate(self.items, (_, id) => {
                search.items.push({
                    score: 1,
                    id
                });
    ...
    const vanilla_removeClasses = (elmts, ...classes) => {
        var norm_classes = classesArray(classes);
        elmts = castAsArray(elmts);
        elmts.map(el => {
            norm_classes.map(cls => {
    ...
    class TomSelect extends(MicroPlugin(MicroEvent)){
        ...
        loadCallback(options, optgroups) {
            ...
            if (!self.loading) vanilla_removeClasses(self.wrapper, self.settings.loadingClass);
        ...
        setActiveItemClass(item) {
            ...
            if (last_active) vanilla_removeClasses(last_active, "last-active");
        ...
        removeActiveItem(item) {
            ...
            vanilla_removeClasses(item, "active");
        }
        clearActiveItems() {
            vanilla_removeClasses(this.activeItems, "active");
            this.activeItems = [];
        }
        ...
        clearActiveOption() {
            if (this.activeOption) {
                vanilla_removeClasses(this.activeOption, "active");
        ...
        refreshOptions(triggerDropdown = true) {
            ...
            for (i = 0; i < n; i++) {
                ...
                for (j = 0, k = optgroups && optgroups.length; j < k; j++) {
                    ...
                    if (j > 0) {
                        ...
                        vanilla_removeClasses(option_el, "active");
            ...
            if (self.settings.highlight) {
                ...
                if (results.query.length && results.tokens.length) utils_iterate(results.tokens, tok => {
        ...
        destroy() {
            ...
            vanilla_removeClasses(self.input, "tomselected", "ts-hidden-accessible");
            self._destroy();
    ...
    function optgroup_columns_plugin_plugin() {
        ...
        self.hook("instead", "onKeyDown", evt => {
            ...
            if (!self.isOpen || !(evt.keyCode === plugin_KEY_LEFT || evt.keyCode === plugin_KEY_RIGHT)) return orig_keydown.call(self, evt);
            self.ignoreHover = true;
            ...
            if (!optgroup) return;
            if (evt.keyCode === plugin_KEY_LEFT) optgroup = optgroup.previousSibling; else optgroup = optgroup.nextSibling;
            if (!optgroup) return;
    ...
    const plugin_escape_html = str => (str + "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
    ...
    const tom_select_complete = TomSelect;
    document.addEventListener("DOMContentLoaded", () => {
        const selectEl = document.querySelector("#filter-sel");
        if (selectEl) new tom_select_complete(selectEl, {
            create: false,
            sortField: {
                field: "text",
                direction: "asc"

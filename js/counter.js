    ...
    function formQuantity() {
        document.addEventListener("click", function (e) {
            ...
            if (targetElement.closest('[data-quantity-plus]') || targetElement.closest('[data-quantity-minus]')) {
                ...
                // Находим actions-блок
                const productActions = quantityBlock.closest('.product')?.querySelector('.product__actions');
                if (productActions) {
                    if (value === 1) {
                        productActions.classList.add('_qty-noactive');
                    } else {
                        productActions.classList.remove('_qty-noactive');
                    }
                }
                // Находим actions-блок
                const productActions2 = quantityBlock.closest('.product-card__info')?.querySelector('.product-info__actions');
                if (productActions2) {
                    if (value === 1) {
                        productActions2.classList.add('_qty-noactive');
                    } else {
                        productActions2.classList.remove('_qty-noactive');
                    }
                }
            }
        });

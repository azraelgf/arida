(() => {
    ...
    function rangeInit(root = document) {
        const sliders = root.querySelectorAll(".range .js-range, .js-range");
        ...
        sliders.forEach(el => {
            ...
            const isDouble = startValues.length > 1;
            const formatter = typeof wNumb === "function" ? wNumb({
                decimals: 0,
                thousand: " "
            }) : {
                to: v => {
            ...
            initialize(el, {
                start: startValues,
                step,
                connect: isDouble ? true : [ true, false ],
                range: {
                    min,
                    max
                },
            ...
            const parseInput = inp => {
                if (!inp) return NaN;
                const n = formatter.from(inp.value ?? "");
                return isNaN(n) ? NaN : clamp(n);
            };
            el.noUiSlider.on("update", values => {
                if (isDouble) {
                    if (inputStart) inputStart.value = values[0];
                    if (inputEnd) inputEnd.value = values[1];
                } else if (inputStart) inputStart.value = values[0];
            });
            const syncFromInputs = () => {
                if (isDouble) {
                    let a = parseInput(inputStart);
                    let b = parseInput(inputEnd);
                    ...
                    const a = parseInput(inputStart);
                    if (!isNaN(a)) el.noUiSlider.set(a);
                }
            };
            const attach = inp => {
                if (!inp) return;
                ...
                inp.addEventListener("blur", syncFromInputs);
